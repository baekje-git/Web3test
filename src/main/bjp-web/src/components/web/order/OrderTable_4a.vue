<template>
  <div class="q-table_wrap"  :style="{ display:setDiaplay }">
    <div class="q-table_header_wrap flex_between">
      <div class="q-table_header_title" @click="setDisplay">
        <p>제품정보</p>
      </div>
    </div>
...
async function setDisplay() {
  if(setDiaplay.value == "none") {
    setDiaplay.value = "block";
  } else {
    setDiaplay.value = "none";
  }
}

bus.on("setDisplay", async () => {
  await setDisplay();
});
</script>
