    ...
    >
      ...
      <template v-slot:body-cell-수량="props">
        <q-td :props="props">
          ...
          >
            <div class="qty_wrap">
              <button type="button" class="btn_qty" @click="minus(props)" tabindex="-1">
              ...
              <div class="">
                <q-input
                  ...
                  @focus="(input) => input.target.select()"
                  @keydown.enter="onEnterKey(props.row)"
                  @keydown.tab="onTabKey(props.row)"
                  :ref="(el) => setQtyInputRef(el, props.row.ITEM_CD)"
                />
              </div>
              <button type="button" class="btn_qty" @click="plus(props)" tabindex="-1">
...
import { ref, inject, computed, onMounted, onBeforeMount, nextTick, toRaw} from "vue";
...
const onEnterKey = async (row) => {
  await nextTick(); // v-model 반영 기다림
  const raw = row.REQ_QTY;
  row.REQ_QTY
  ...
  // await bus.emit("refreshRetrunFocus");
...
const onKey = async (evt) => {
  ...
  if (evt.key === "ArrowUp") {
    ...
    if(nextIndex <computedRows.length ){
      nextIndex++;
    }
    /*
    do {
...
const scrollToRow = (index) => {
  ...
  const rowElement = rowRefs.value[row.ITEM_CD];
