...
//  alert(selected.value[0].SN);
var snArray = [];
for(var i=0 ; i < selected.value.length; i++){
  // console.log("i::"+ selected.value[i].SN);
  // snArray += selected.value[i].SN;
  snArray.push(selected.value[i].SN);
}
  const param = {
    /* 레포트 파라미터 예시*/
    dataset_0: {},
...
async function exportPrint(row) {
  ...
  await api.put("/ordLedger/upatePrintOrder", upatePrintOrder);
