<template>
          ...
          <BtnComp_br_purple btn_br_top="카드회사" btn_br_bottom="공지사항">
            <q-tooltip anchor="top right" self="top left">
              <p style="font-size: 14px !important">
                정확한 카드번호와 유효기간을 확인하신 후 결제 부탁드립니다.
              </p>
            </q-tooltip>
          </BtnComp_br_purple>
          -->
  ...
  <q-form
    id="kakaoForm"
    action="http://localhost:9050/inicis/reqPayment"
    method="post"
    target="PayForm"
  >
...
const filteredcardGbOptions = computed(() => {
  ...
  if (["58", "59", "61"].includes(branchCode)) {
...
async function makeNewWindow() {
  ...
  frm.action = url; // 개발 및 운영 URL - 로컬에서 테스트시, 하기의 로컬 URL을 사용하거나 vue project 빌드 후 9050으로 접속하여 진행
  //frm.action = "http://localhost:9050/payment/wpaystdPayAuthCardRequest"; // 로컬 URL
  frm.target = "IfWindow";
  frm.method = "POST";
  frm.submit();
}
...
*/
    ...
    if (paybyRadio.value === "1") {
      ...
      if (cardGb === "INICIS" && birthDt === "" && userInfo.BRCH_CD !== "53") {
