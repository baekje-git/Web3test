...
public class PaymentController {
	...
	public ResponseEntity<Object> insertPayment(@RequestBody HashMap<String, Object> hMap) throws Exception {
		...
		if (custData.size() > 0) {
			...
			// 20250814 리인팜은 본인인증사용안하기때문에 설정 값을 넘기기위해 설정
			infoMap.put("brchCd", String.valueOf(custHm.get("BRCH_CD")));	
		...
		if (cardGb.equals("INICIS")) {
			...
			if (recvMap.get("resultCode").equals("00")) {
				...
				//paymentService.insertResultPayment(dataHm);
			   	  ...
			   	  int creditcard_seq_no = paymentService.insertResultPayment(dataHm);
			   	  procMap.put("CREDITCARD_SEQ_NO"   , creditcard_seq_no);
			   	  log.info("55555");
			   	  System.out.println("55555");
			       try {
			    	   cmmnDao.callProcedure("payment.procPayCreditCard",procMap);  
			 		} catch (Exception e) {
...
//			log.info("HTTPClientByPost result : "+rspStr);
			...
			String TID				= String.valueOf(recvMap.get("tid"));             //이니시스에서 발행한 TID
